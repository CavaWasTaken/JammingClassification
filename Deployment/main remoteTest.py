...
def main():
    ...
    path_to_spec = "../NMon_Dataset_10MHz/image_dataset"
    ...
    try:
        ...
        while True:
            ...
            # extract a random number from 0 to 5
            class_num = np.random.randint(0, 6)

            # read the class_num folder in the path_to_spec and randomly select a class folder
            class_name = class_names[class_num]
            class_folder = os.path.join(path_to_spec, class_name)

            if class_name == 'CLEAN':
                # select a random file from the class_folder
                spec_file = np.random.choice(os.listdir(class_folder))
                spec_path = os.path.join(class_folder, spec_file)
                spec = np.load(spec_path)
            else:
                # select a random folder from the class_folder
                subfolder = np.random.choice(os.listdir(class_folder))
                subfolder_path = os.path.join(class_folder, subfolder)
                # select a random file from the subfolder
                spec_file = np.random.choice(os.listdir(subfolder_path))
                spec_path = os.path.join(subfolder_path, spec_file)
                spec = np.load(spec_path)
                # by extracting the class and the power of the spectogram from the filename, we can extract the exact features from the features_df
